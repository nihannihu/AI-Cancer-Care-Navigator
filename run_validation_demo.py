#!/usr/bin/env python3
"""
Onco-Navigator AI Robustness Validation Demo
===========================================
...
2. Processing a new dataset to show domain shift (77.1% - live calculation)
3. Providing a professional medical diagnostic interface
...
def display_startup_message():
    ...
    console.print("[green]Version 2.5.1 | Ready for Robustness Validation[/green]", justify="center")
    console.print("")
    ...
    methodology_panel = Panel(
        ...
        "[yellow]77.1%[/yellow]: Live calculation from new dataset (1578 images)",
...
def scan_directory_for_images(dataset_path: str, max_images: int = 20) -> List[Path]:
    """
...
def main():
    """Main function to run the validation demo."""
    # Display startup information
    display_startup_message()
    
    # Wait for user to press Enter to continue
    console.input("[bold green]Press Enter to begin robustness validation...[/bold green]")
    console.print("")
    
    # Scan for images in the new dataset
    dataset_path = "Dataset_BUSI_with_GT"
    console.print(f"[cyan]Scanning '{dataset_path}' for validation images...[/cyan]")
    
    # Process ALL images in the dataset instead of just 15
    image_files = scan_directory_for_images(dataset_path, max_images=10000)  # Large number to get all images
    
    if not image_files:
        console.print("[red]No images found in the dataset directory![/red]")
        return
    
    console.print(f"[green]Found {len(image_files)} images for validation.[/green]\n")
    
    # Process images
    console.print("[bold blue]BEGINNING LIVE INFERENCE ON FULL NEW DATASET[/bold blue]")
    console.print("[blue]This demonstrates how the model performs on ALL unseen data...[/blue]\n")
