...
async def chat(
    ...
    return {"response": response_text, "intent": intent_data}
...
async def upload_report(
    ...
    # 2. Analyze
    analysis = await lab_analyzer.analyze_report(text)
...
async def get_dashboard(current_user: dict = Depends(get_current_user)):
    ...
    if pcp_cases_list:
        try:
            ...
            for case in pcp_cases_list:
                timeline.append({
                    "date": case.get("timestamp", "Unknown"),
                    "type": "AI Analysis",
    ...
    if not timeline:
        timeline = [{"date": "N/A", "type": "Info", "details": "No medical history available yet"}]
    
    # 3. Get REAL AI insights from their case
    if pcp_cases_list:
        # Use the most recent case for insights (assuming they're sorted by timestamp)
        latest_case = pcp_cases_list[0]  # Most recent should be first
        insights = {
            "risk_score": latest_case.get("risk_score", 0),
            "survival_insight": f"Based on the latest AI analysis, the patient has been classified as {latest_case.get('risk_label', 'Unknown')} risk.",
        ...
        # Default insights if no case found
        insights = {
            "risk_score": 0,
